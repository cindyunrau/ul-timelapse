...
# todo cmd line out is 
# Printer name
# Current Print: Name, progress (or NONE)
HTTP = "http://"
...
def connection_error(err): # todo only catch connection errors and set timeout (not inf)
...
def start_print(tmpdir, job_name, job_uuid):
    logging.info('New Print Started:')
    logging.info('File = %s, UUID = %s' % (job_name, job_uuid))
...
def end_print(path,name):
    ...
    ffmpegcmd = f"ffmpeg -loglevel level+warning -r 30 -i {in_files} {out_file}"
...
def main():
    ...
    print('Starting Timelapse.py - PRINTER NAME = \"%s\", HOST = %s' % (NAME, HOST))
    logging.info('Starting Timelapse.py - PRINTER NAME = \"%s\", HOST = %s' % (NAME, HOST))
    ...
    if job_state != None:
        ...
        if printer_status == "printing" and job_state == "printing":
            job_path = os.path.join(directory, job_uuid)
            start_print(job_path,job["name"],job_uuid)
